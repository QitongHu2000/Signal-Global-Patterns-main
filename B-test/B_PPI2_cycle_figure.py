...
class cycle_figure:
    ...
    def cycle_plotting(self, path, vector, theory_time, colors):
        ...
        ax.scatter(0, 0, s=self.d[self.source]**(1.5)+200, c=colors[1],edgecolor='black',linewidth = 1.5,zorder=3)
...
class signal_propagation:
    ...
    def simulation(self,A):
        def F(A,x):
            return np.mat(self.B-self.C*x-self.alpha*np.multiply(x,A*x))
        ...
        def sim_first(A):
            ...
            sol=solve_ivp(Fun, [0,self.t_0], x_0, rtol=1e-13, atol=1e-13) #odeint(Fun,x_0,t,args=(A,a,b))
            xs=sol.y.T
            t=sol.t
            x=xs[-1,:].tolist()
            return x, t[-1]
        ...
        def sim_second(A,x,t_0,source):
            x[source]*=(1+self.gamma)
            sol=solve_ivp(Fun_1, [self.t_0,self.t_0+self.t_1], x, rtol=1e-13, atol=1e-13, args=(source,))#odeint(Fun_1,x,t,args=(A,a,b,source),atol=1e-13,rtol=1e-13)
            xs=sol.y.T
            ts=sol.t
...
if __name__ == '__main__':
    ...
    theory_time_hens = load_dict('B_PPI2_global_degree_hens_'+str(int(100*B))+'_'+str(int(100*C))).tolist()[0]
    theory_time_multi = load_dict('B_PPI2_global_degree_multi_hens_'+str(int(100*B))+'_'+str(int(100*C))).tolist()[0]
    theory_time_ours = load_dict('B_PPI2_global_degree_multi_'+str(int(100*B))+'_'+str(int(100*C))).tolist()[0]
