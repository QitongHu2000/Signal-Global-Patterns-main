...
class cycle_figure:
    ...
    def nodes_positioning(self):
        ...
        for k in range(kernel_layer-1, -1, -1):
            ...
            for node in index_sorted[k]:
                ...
                if(len(path_dict[node])>2):
                    r = (self.theory_time[node]-self.theory_time[path_dict[node][-2]])**4+(self.theory_time[path_dict[node][-2]])**self.dis#np.sum([np.log(d[dot]) for dot in path_dict[node][:-1]])
                else:
                    r = self.theory_time[node]**self.dis
    ...
    def cycle_plotting(self, path, vector, theory_time, colors):
        ...
        for node in H.nodes:
            ...
            s=np.min([1600,self.d[node]**1.8+200])
        ...
        ax.scatter(0, 0, s=np.min([1600,self.d[node]**1.8+200]), c=colors[1],edgecolor='black',linewidth = 1.5,zorder=3)
