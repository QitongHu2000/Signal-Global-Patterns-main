...
def simulation(A):
    def F(A,x):
        return np.mat(B*np.multiply(x,1-np.power(x,a)/C)+alpha*np.multiply(x,A*(x/(1+x))))
...
def R(x):
    return B*(np.power(x,a)/C-1)

def W(x):
    return 1/(B*(np.power(x,a)/C-1))

def H2(x):
    return alpha * x/(1+x)

def R_(x):
    return B*a*np.power(x,a-1)/C

def W_(x):
    return -1*(B*a*np.power(x,a-1)/C)/(B*(np.power(x,a)/C-1))**2

def H2_(x):
    return alpha/(1+x)**2

def C_(x):
    return alpha/(1+x)**2/(B*a*np.power(x,a-1)/C)
...
def calculate_Wij(H,degrees,xs):
    ...
    for (u,v) in H.edges:
        count+=1
        H.edges[u,v]['weight_hens']=float(degrees[v,0])**(-1.0)
...
# savemat('../Networks/BA1.mat',{'A':A_edge})
...
k=1
sources=[141, ] #np.random.choice(G_edge.nodes,k) #7
...
save_dict(np.mat(times_simulation_times), 'M_BA_global_time_'+str(int(100*B))+'_'+str(int(100*C)))
save_dict(np.mat(theory_times_hens), 'M_BA_global_degree_hens_'+str(int(100*B))+'_'+str(int(100*C)))
save_dict(np.mat(theory_times_L), 'M_BA_global_degree_L_'+str(int(100*B))+'_'+str(int(100*C)))
save_dict(np.mat(theory_times_multi_hens), 'M_BA_global_degree_multi_hens_'+str(int(100*B))+'_'+str(int(100*C)))
save_dict(np.mat(theory_times_multi), 'M_BA_global_degree_multi_'+str(int(100*B))+'_'+str(int(100*C)))
save_dict(np.mat(theory_times_multi_weight), 'M_BA_global_degree_multi_weight_'+str(int(100*B))+'_'+str(int(100*C)))