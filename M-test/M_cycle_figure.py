...
class signal_propagation:
    ...
    def simulation(self,A):
        def F(A,x):
            return np.mat(self.B*np.multiply(x,1-np.power(x,self.a)/self.C)+self.alpha*np.multiply(x,A*(x/(1+x))))
...
if __name__ == '__main__':
    ...
    theory_time_ours = load_dict('M_BA_global_degree_multi_'+str(int(100*B))+'_'+str(int(100*C))).tolist()[0]

    signal=signal_propagation(t_selected)
    scale,xs=signal.simulation(A)

    colors3 = ['#EECB8E', '#DC8910']
    path3 = 'M_BA_{}_distance_network_ours.pdf'.format(int(100*t_selected))
    dis3=1
