...
class cycle_figure:
    ...
    def cycle_plotting(self, path, vector, theory_time, colors):
        ...
        ax.scatter(0, 0, s=self.d[self.source]**(1.5)+100, c=colors[1],edgecolor='black',linewidth = 1.5,zorder=3)
...
class signal_propagation:
    ...
    def simulation(self,A):
        def F(A,x):
            return np.mat(-self.B*np.power(x,self.a)+self.alpha*A*(np.power(x,self.b)/(1+np.power(x,self.b))))
...
if __name__ == '__main__':
    ...
    theory_time_ours = load_dict('R_BA_global_degree_multi_weight_'+str(int(100*a))+'_'+str(int(100*b))).tolist()[0]

    signal=signal_propagation(t_selected)
    scale,xs=signal.simulation(A)
    
    colors3 = ['#ABBAE1', '#526D92']
    path3 = 'R_BA_{}_distance_network_ours.pdf'.format(int(100*t_selected))
    dis3=1
