...
def calculate_Wij(H,degrees,xs):
    ...
    for u in H.nodes:
        ...
        for v in list(dict(H[u]).keys()):
            r_root+=(xs[-1,u]*W_(xs[-1,u])*H2(xs[-1,v]))
        r_roots[u]=r_root
    ...
    for (u,v) in H.edges:
        ...
        H.edges[u,v]['Rij']=np.abs(W(xs[-1,u])*H2_(xs[-1,v])*xs[-1,v]/r_roots[u])
        H.edges[u,v]['Cij']=H2_(xs[-1,v])/R_(xs[-1,u])
    return H
    ...
    for (k,u) in enumerate(H.nodes):
        ...
        for path in paths:
            ...
            for i in range(len(path)-3,-1,-1):
                weight[i]=1+C_(xs_edge[-1,path[i]])*C_(xs_edge[-1,path[i+1]])/weight[i+1]
...
k=1
sources=[1641, ] #np.random.choice(G_edge.nodes,k) #[196, 55, 107]
...
for source in sources:
    ...
    values_hens, values_L=calculate_shortest_path(H_edge,source)
    values_multi_hens, values_multi, values_multi_weight = calculate_multi_path(H_edge,source)
    
    theory_time_hens=[values_hens[i] for i in values_hens]
    theory_time_L=[values_L[i] for i in values_L]
...
save_dict(np.mat(times_simulation_times), 'E_UCIonline_global_time_'+str(int(100*B)))
save_dict(np.mat(theory_times_hens), 'E_UCIonline_global_degree_hens_'+str(int(100*B)))
save_dict(np.mat(theory_times_L), 'E_UCIonline_global_degree_L_'+str(int(100*B)))
save_dict(np.mat(theory_times_multi_hens), 'E_UCIonline_global_degree_multi_hens_'+str(int(100*B)))
save_dict(np.mat(theory_times_multi), 'E_UCIonline_global_degree_multi_'+str(int(100*B)))
save_dict(np.mat(theory_times_multi_weight), 'E_UCIonline_global_degree_multi_weight_'+str(int(100*B)))