...
if __name__ == '__main__':
    ...
    theory_time_hens = load_dict('E_UCIonline_global_degree_hens_'+str(int(100*B))).tolist()[0]
    theory_time_multi = load_dict('E_UCIonline_global_degree_multi_hens_'+str(int(100*B))).tolist()[0]
    theory_time_ours = load_dict('E_UCIonline_global_degree_multi_'+str(int(100*B))).tolist()[0]

    signal=signal_propagation(t_selected)
    scale,xs=signal.simulation(A)
    
    colors1 = ['#8DDFBF', '#328CA0']
    path1 = 'E_UCIonline_{}_distance_network_hens.pdf'.format(int(100*t_selected))
    dis1=1
    figure1 = cycle_figure(G, source, dis1, eta)
    figure1.cycle_plotting(path1, scale, theory_time_hens, colors1)
    
    colors2 = ['#ABBAE1', '#526D92']
    path2 = 'E_UCIonline_{}_distance_network_multi.pdf'.format(int(100*t_selected))
    dis2=2
    figure2 = cycle_figure(G, source, dis2, eta)
    figure2.cycle_plotting(path2, scale, theory_time_multi, colors2)
    
    colors3 = ['#EECB8E', '#DC8910']
    path3 = 'E_UCIonline_{}_distance_network_ours.pdf'.format(int(100*t_selected))
    dis3=2
